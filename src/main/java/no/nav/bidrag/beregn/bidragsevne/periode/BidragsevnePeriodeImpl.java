...
public class BidragsevnePeriodeImpl implements BidragsevnePeriode {
  ...
  public BeregnBidragsevneResultat beregnPerioder(BeregnBidragsevneGrunnlag beregnBidragsevneGrunnlag) {
    ...
    if ((perioder.size() > 1) &&
        (perioder.get(perioder.size() - 2).getDatoTil().equals(beregnBidragsevneGrunnlag.getBeregnDatoTil())) &&
    ...
    for (Periode beregningsperiode : perioder) {
      ...
      var skatteklasse = justertSkatteklassePeriodeListe.stream()
          ...
          .findFirst()
          .orElseThrow(
              () -> new IllegalArgumentException("Grunnlagsobjekt SKATTEKLASSE mangler data for periode: " + beregningsperiode.getPeriode()));

      var bostatus = justertBostatusPeriodeListe.stream().filter(i -> i.getPeriode().overlapperMed(beregningsperiode))
          .map(bostatusPeriode -> new Bostatus(bostatusPeriode.getReferanse(), bostatusPeriode.getBostatusKode()))
          .findFirst()
      ...
      var saerfradrag = justertSaerfradragPeriodeListe.stream()
          .filter(i -> i.getPeriode().overlapperMed(beregningsperiode))
          .map(saerfradragPeriode -> new Saerfradrag(saerfradragPeriode.getReferanse(), saerfradragPeriode.getSaerfradragKode()))
          .findFirst()
          .orElseThrow(() -> new IllegalArgumentException("Grunnlagsobjekt SAERFRADRAG mangler data for periode: " + beregningsperiode.getPeriode()));
      ...
      var beregnBidragsevneGrunnlagPeriodisert = new GrunnlagBeregning(inntektListe, skatteklasse, bostatus, barnIHusstand, saerfradrag,
          sjablonliste);
