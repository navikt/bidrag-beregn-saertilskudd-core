...
class SaertilskuddPeriodeTest {
  ...
  private final SaertilskuddPeriode saertilskuddPeriode = SaertilskuddPeriode.getInstance();
  ...
  void testPaaPeriode() {
    ...
    var bidragsevnePeriode = new BidragsevnePeriode(TestUtil.BIDRAGSEVNE_REFERANSE, new Periode(LocalDate.parse("2017-01-01"),
        LocalDate.parse("2019-07-01")), BigDecimal.valueOf(11000));
    var bidragsevnePeriodeListe = new ArrayList<BidragsevnePeriode>();
    bidragsevnePeriodeListe.add(bidragsevnePeriode);

    bidragsevnePeriode = new BidragsevnePeriode(TestUtil.BIDRAGSEVNE_REFERANSE, new Periode(LocalDate.parse("2019-07-01"),
        LocalDate.parse("2020-01-01")), BigDecimal.valueOf(11069));
    bidragsevnePeriodeListe.add(bidragsevnePeriode);

    var bPsAndelSaertilskuddPeriode = new BPsAndelSaertilskuddPeriode(TestUtil.BPS_ANDEL_SAERTILSKUDD_REFERANSE,
        new Periode(LocalDate.parse("2017-01-01"), LocalDate.parse("2020-01-01")), BigDecimal.valueOf(60.6), BigDecimal.valueOf(4242), false);
    ...
    var samvaersfradragPeriode = new SamvaersfradragGrunnlagPeriode(TestUtil.SAMVAERSFRADRAG_REFERANSE, 1,
        new Periode(LocalDate.parse("2019-01-01"), LocalDate.parse("2020-01-01")), BigDecimal.valueOf(457));
    ...
    BeregnSaertilskuddGrunnlag beregnSaertilskuddGrunnlag =
        new BeregnSaertilskuddGrunnlag(beregnDatoFra, beregnDatoTil, 1, bidragsevnePeriodeListe, bPsAndelSaertilskuddPeriodeListe,
            lopendeBidragPeriodeListe, samvaersfradragPeriodeListe, TestUtil.byggSjablonPeriodeListe());
    ...
    assertAll(
        ...
        () -> assertThat(resultat.getResultatPeriodeListe().get(0).getResultat().getResultatkode()).isEqualTo(ResultatKode.SAERTILSKUDD_INNVILGET));

    printGrunnlagResultat(resultat);
  }

  private void printGrunnlagResultat(BeregnSaertilskuddResultat beregnSaertilskuddResultat) {
    beregnSaertilskuddResultat.getResultatPeriodeListe().stream()
        .sorted(Comparator.comparing(pR -> pR.getPeriode().getDatoFom()))
        .forEach(sortedPR -> System.out.println("Dato fra: " + sortedPR.getPeriode().getDatoFom() + "; " + "Dato til: "
            + sortedPR.getPeriode().getDatoTil() + "; " + "Resultat: " + sortedPR.getResultat().getResultatBelop()
